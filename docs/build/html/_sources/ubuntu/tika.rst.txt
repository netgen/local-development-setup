...
Apache Tika is a content analysis toolkit used to detect and extract
metadata and text from different file types. It can be used both as a
service and a command line utility.

1 Install
---------

In the console, create ``jars`` directory in your home directory and
position into it:
...
appropriate version to the created ``jars`` directory from
...
The server will be available on ``127.0.0.1`` on port ``9998``. To find
